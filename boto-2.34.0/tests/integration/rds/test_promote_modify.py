...
class PromoteReadReplicaTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parentDB_name = "boto-db-%s" % str(int(time.time()))
        self.replicaDB_name = "replica-%s" % self.parentDB_name
        self.renamedDB_name = "renamed-replica-%s" % self.parentDB_name

  
    def tearDown(self):
        instances = self.conn.get_all_dbinstances()
        for db in [self.parentDB_name, self.replicaDB_name, self.renamedDB_name]:
            for i in instances:
                if i.id == db:
    ...
    def test_promote(self):
        ...
        self.parentDB = self.conn.create_dbinstance(self.parentDB_name, 5, 'db.t1.micro', 'root', 'bototestpw')
        
        # Wait up to 15 minutes for the parentDB to become available
        print '--- waiting for "%s" to become available  ---' % self.parentDB_name
        wait_timeout = time.time() + (15 * 60)
        time.sleep(60)
 
        instances = self.conn.get_all_dbinstances(self.parentDB_name)
        inst = instances[0]
        ...
        self.replicaDB = self.conn.create_dbinstance_read_replica(self.replicaDB_name, self.parentDB_name)
        ...
        # Verify that the parent no longer has any read replicas
        instances = self.conn.get_all_dbinstances(self.parentDB_name)
        inst = instances[0]
